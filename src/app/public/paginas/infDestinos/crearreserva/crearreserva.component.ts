...
export class CrearreservaComponent implements OnInit {
  ...
  // AGREGAR: Debug del estado de autenticación
  private debugAuthState(): void {
    ...
    if (user) {
      try {
        ...
        console.log('🆔 ID Usuario:', parsedUser.idUsuario);
    ...
    if (token) {
      try {
        ...
        console.log('🆔 ID en JWT:', payload.idUsuario);
        console.log('📧 Email en JWT:', payload.email || payload.sub);
  ...
  // AGREGAR: Método para decodificar JWT
  ...
  private cargarPaquete(): void {
    this.reservaService.obtenerPaquetePorId(this.paqueteId!).subscribe({
      next: data => {
        ...
        this.paquete = data.data ?? data;

        if (!this.paquete.proveedor?.telefono) {
          console.warn('⚠️ El paquete no tiene número de teléfono del proveedor');
        }

        this.fechaInicio = this.paquete.fechaInicio
          ? new Date(this.paquete.fechaInicio).toISOString().split('T')[0]
          : '';
        this.fechaFin = this.paquete.fechaFin
          ? new Date(this.paquete.fechaFin).toISOString().split('T')[0]
          : '';
      },
      error: err => {
  ...
  // ACTUALIZAR: Método cargarUsuario con más debug
  ...
  confirmarReserva(): void {
    ...
    console.log('📦 Tipo Reserva:', this.tipoReserva);
    console.log('👥 Cantidad Personas:', this.cantidadPersonas);

    // Verificar usuario ID
    if (!this.usuarioId || this.usuarioId === 0) {
      console.error('❌ ERROR: usuarioId inválido');
    ...
    if (this.tipoReserva === 'PAQUETE' && this.paqueteId) {
      this.reservaService.obtenerPaquetePorId(this.paqueteId).subscribe({
        next: data => {
          ...
          this.paquete = data.data ?? data; // ✅ Asegura acceso correcto a los campos
        },
      ...
      this.reservaService.obtenerActividadPorId(this.actividadId).subscribe({
        next: data => {
          ...
          this.actividad = data.data ?? data; // ✅ También aquí
        },
