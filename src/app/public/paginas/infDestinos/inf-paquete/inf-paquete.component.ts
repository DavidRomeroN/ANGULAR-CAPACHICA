...
export class InfPaqueteComponent implements OnInit {
  ...
  cargarPaquete(id: number): void {
    this.paquetesService.getById(id).subscribe({
      next: response => {
        ...
        this.paquete = response.data; // ✅ solo los datos del paquete
        console.log('Datos reales asignados a paquete:', this.paquete);
      },
  ...
  onReservaExitosa(): void {
    ...
    this.paquetesService.getById(id).subscribe({
      next: data => {
        this.paquete = data.data ?? data; // ✅ Esto extrae correctamente los datos si vienen anidados
